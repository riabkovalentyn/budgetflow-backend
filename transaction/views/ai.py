...
class AIAdviceView(APIView):
    ...
    def get(self, request):
        # Mirror POST but without prompt in body
        ...
        try:
            ...
            txs = list(
                Tx.objects(user_id=user_id).order_by('-created_at')[:50]
            )
            payload = {
                'transactions': [
                    {
                        ...
                        'created_at': (
                            ...
                            if getattr(t, 'created_at', None) else None
                        ),
                    }
                    for t in txs
                ],
